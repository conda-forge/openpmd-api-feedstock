...
if [[ ${target_platform} =~ osx.* ]]; then
    ...
    #   2) patch
    echo "Patching..."
    sed -i "s@${CONDA_BUILD_SYSROOT}/usr/lib/libdl.tbd@-ldl@g"             $PREFIX/lib/cmake/openPMD/openPMDTargets.cmake
    sed -i "s@${CONDA_BUILD_SYSROOT}/usr/lib/libm.tbd@-lm@g"               $PREFIX/lib/cmake/openPMD/openPMDTargets.cmake
    sed -i "s@${CONDA_BUILD_SYSROOT}/usr/lib/libpthread.tbd@-lpthread@g"       $PREFIX/lib/cmake/openPMD/openPMDTargets.cmake
    sed -i "s@${CONDA_BUILD_SYSROOT}/usr/lib/libz.tbd@-lz@g" $PREFIX/lib/cmake/openPMD/openPMDTargets.cmake
